...
module.exports.login = R.curry(function(email, password, user) {
	...
	if(utility.isPlainObject(email))
		authObject = email
...
module.exports.logout = R.curry(function(authtoken, user){
	if(authtoken)
		user.app = user.app.setAuthToken(authtoken)
	
	if(!authtoken && !user.app.isAuthenticated()){      // checks whether authtoken is set
...
instanceMethodBuilder.define('logout', 2, [null]);
...
* @function anyAuth
* @static
* @param fnUrl {String} The name of the AnyAuth function that needs to be executed
...
module.exports.resetPassword = R.curry(function(password, confirm_password, resetPassToken, user){
		...
		if(utility.isPlainObject(password))
			resetObj = password
...
instanceMethodBuilder.define('resetPassword', 4, [null, null]);
/**
